...
Una decisión de diseño relevante en el script es que \texttt{building\_id} se trata como texto en el pipeline, pese a ser en el dateset de origen un identificador numérico:

\begin{verbatim}
telemetria["building_id"] = telemetria["building_id"].astype(str)
\end{verbatim}

Esta misma decisión se refuerza aguas abajo en el simulador de telemetría: el payload que emite cada medidor por MQTT se publica como JSON en texto plano.
...
La decisión de diseño más relevante es publicar cada evento con un formato JSON en texto plano. El protocolo MQTT lo permite, ya que es agnóstico al formato del payload: no impone ninguna estructura ni codificación, y transporta indistintamente JSON, texto plano o binario.
