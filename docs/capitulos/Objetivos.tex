...
\textbf{Acciones}: instrumentar el simulador MQTT y desplegar Mosquitto como broker MQTT con un microservicio de bridge MQTT-Kafka que enviará los eventos ingeridos al tópico de Kafka correspondiente; definir el nivel de calidad de servicio (QoS) y la topología de tópicos; medir la latencia máxima de extremo a extremo.
...
\textbf{KPI}: cada micro-lote de Spark Structured Streaming se procesa en menos de 1 segundo sin acumular retraso entre micro-lotes sucesivos.

\textbf{Acciones}: implementar el proceso de Spark Structured Streaming (DataFrame API) con ventanas de agregación temporal y \textit{watermarking} para el tratamiento de datos tardíos; medir la máxima duración de procesamiento cada micro-lote.
