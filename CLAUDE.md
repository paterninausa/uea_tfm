...
Funcionando: stack completo en Docker; preparación de datos (tres Parquet: hechos, dimensión de 498 edificios y línea base de 652 sensores), con **reubicación temporal al presente vía `prepare_ashrae.py --fecha-final`**; esquema Avro v1 registrado con sus reglas; simulador con escalera de carga (`--max-sensors`), selección de cola para la demo (`--ultimas-semanas`) y rebase temporal solo para medición (`--traer-a`, exclusivo de `load_ladder`); bridge con validación y DLQ; job de Spark con broadcast join contra las dos tablas de referencia, agregación por ventana y escritura idempotente en ambos sumideros; tres dashboards de Grafana aprovisionados declarativamente.
